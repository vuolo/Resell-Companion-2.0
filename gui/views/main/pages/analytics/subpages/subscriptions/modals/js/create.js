...
window.resetModalOptions = () => {
  if (window.createApp) {
    if (window.createApp.activeSubscriptionIndex != -1 && window.modalOptions.sale.tracking.isTracking) window.parent.refreshTracking(window.createApp.activeSubscriptionIndex, true);
    window.createApp.activeSubscriptionIndex = -1;
    resetSuggestedItems();
...
window.createApp = new Vue({
  ...
  methods: {
    ...
    applySuggestedItem: async function(item) {
      ...
      window.parent.preloadSubscriptions360Media(modalOptions);
    },
    ...
    finalizeModal: function() {
      ...
      modalOptions.suggestions.isSearchingForSizes = false;
      if (this.activeSubscriptionIndex == -1) window.parent.addSubscription();
      else window.parent.updateSubscription(this.activeSubscriptionIndex);
      this.closeModal();
    },
