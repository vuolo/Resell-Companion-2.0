...
window.resetModalOptions = () => {
  if (window.createApp) {
    if (window.createApp.activeCardIndex != -1 && window.modalOptions.sale.tracking.isTracking) window.parent.refreshTracking(window.createApp.activeCardIndex, true);
    window.createApp.activeCardIndex = -1;
    resetSuggestedItems();
...
window.createApp = new Vue({
  ...
  methods: {
    ...
    applySuggestedItem: async function(item) {
      ...
      window.parent.preloadCards360Media(modalOptions);
    },
    ...
    finalizeModal: function() {
      ...
      modalOptions.suggestions.isSearchingForSizes = false;
      if (this.activeCardIndex == -1) window.parent.addCard();
      else window.parent.updateCard(this.activeCardIndex);
      this.closeModal();
    },
...
async function refreshSuggestedItems(inputtedName = modalOptions.name) {
  ...
  let searchResults = await window.parent.parent.parent.marketAPI.searchMarketplace('stockx', inputtedName, { isTradingCard: true });
