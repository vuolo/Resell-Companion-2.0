...
window.deleteApp = new Vue({
  ...
  methods: {
    ...
    finalizeModal: function() {
      for (var card of window.modalOptions.cards) window.parent.removeCard(window.parent.getCardByID(card.id), false);
      if (window.modalOptions.cards.length > 0) { // do the if statement in case user presses enter on cards page
        window.parent.cardsApp.applyDateSearch();
        this.closeModal();
      }
    },
    closeModal: function() {
      ...
      window.resetModalOptions();
      if (window.parent.getSelectedCards().length == 1) window.parent.setAllCardsSelected(false, false);
    }
  }
});
